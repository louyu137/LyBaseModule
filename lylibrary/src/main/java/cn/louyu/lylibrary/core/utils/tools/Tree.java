...
public class Tree<T> {
     ...
     * */
    private List<Tree<T>> preVisit(Tree<T> tree){
     ...
     * （2）.访问根结点；
     * */
    private List<Tree<T>> sufVisit(Tree<T> tree){
        List<Tree<T>> values=new ArrayList<Tree<T>>();
        if (tree.isEmpty()) return values;
        for(int i=0;i<childs.size();i++){
     ...
     * (4) 重复上面的操作
     * */
    private List<Tree<T>> levelVisit(Tree<T> tree){
        List<Tree<T>> values=new ArrayList<Tree<T>>();
