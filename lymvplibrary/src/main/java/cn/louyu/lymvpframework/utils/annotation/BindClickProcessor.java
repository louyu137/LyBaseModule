...
public class BindClickProcessor implements ProcessorIntf<Method> {
    @Override
    public boolean accept(AnnotatedElement e) {
        //这个很简单，就是告诉管理器我响应OnClick注解
        return e.isAnnotationPresent(BindClick.class);
    }
    ...
    public void process(Object object, View view, Method method) {
        if(view==null) return;
    	//先拿到具体的注解对象 ，并拿到里面的值
        final BindClick oc = method.getAnnotation(BindClick.class);
